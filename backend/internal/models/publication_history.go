...
func (db *DB) NodePublished(publication string, nodeID int64, computingID string) error {
	var userID int64
	...
	qs := "insert into publication_history (publication, node_id, user_id, published_at) values (?,?,?,NOW())"
	_, err := db.Exec(qs, publication, nodeID, userID)
	return err
}

// GetPublicationHistory returns all of the publication history for a node
func (db *DB) GetPublicationHistory(publication string, nodeID int64) ([]PublicationHistory, error) {
	log.Printf("Get publication history for %d to %s", nodeID, publication)
...
func (db *DB) GetLatestPublication(publication string, nodeID int64) *time.Time {
	log.Printf("Get latest publication info for %d to %s", nodeID, publication)
	q := "select published_at from publication_history where node_id=? and publication=? order by published_at desc limit 1"
	var ts *time.Time
	db.QueryRow(q, nodeID, publication).Scan(&ts)
	return ts
}
